...
## Solution
```
Browser: http://www.pythonchallenge.com/pc/def/equality.html
...
import re                                               # from re import *, then don't need use re.

# Fetch the page
url = "http://www.pythonchallenge.com/pc/def/equality.html"
html = urlopen(url).read().decode('utf-8')
mess = html.split("<!--")[1][:-4]                       # if not use mess use the uncomment text

# Extract the comment block where the hidden text is
# text = re.search(r'<!--(.*?)-->', mess, re.S).group(1)

# Find all lowercase letters with exactly 3 uppercase on each side, Note: Need outer [^A-Z] parts are there to make the regex stricter
...
Browser:http://www.pythonchallenge.com/pc/def/linkedlist.html
linkedlist.php

Browser: http://www.pythonchallenge.com/pc/def/linkedlist.php 🔐

Method 2:
Use a counter for the char then test the 6 conditions
    counter in ascii_lowercase
    counter_left_1      counter_right_1 in ascii_uppercase
    counter_left_2      counter_right_2
    counter_left_3      counter_right_3
```
