...
Level: Beginner
What I learned:
    list has append('0x' + hex(memoryAddress)[2:].zfill(4) + '  ')
"""
...
with open('data/sevenletterwords.txt') as word_list_file:
    WORDS = word_list_file.readlines()

for i in range(len(WORDS)):
    # Convert each word to uppercase and remove the trailing newline ❤️
...
def getWords():
    ...
    words = [secret_password]                    # 1st word is the secret_password

    # Find two more words; these have zero matching letters, use "< 3" because secret password is already in words
    while len(words) < 3:
        randomWord = getOneWordExcept(words)

        if numMatchingLetters(secret_password, randomWord) == 0:
    ...
    for i in range(500):
        ...
        randomWord = getOneWordExcept(words)
        if numMatchingLetters(secret_password, randomWord) == 3:
            words.append(randomWord)
