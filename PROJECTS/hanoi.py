...
""")
    ...
    def askForPlayerMove(self, towers):
    ...
    def displayTowers(self, towers):
        ...
        for level in range(TOTAL_DISKS, -1, -1):
            for tower in (towers['A'], towers['B'], towers['C']):
                if level >= len(tower):
                    self.displayDisk(0)              # Display the bare pole with no disk
                else:
                    self.displayDisk(tower[level])   # Display the disk
            print()
