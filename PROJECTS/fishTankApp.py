...
Similar to ASCIIQuarium or @EmojiAquarium, but based on an
older ASCII fish tank program for DOS.

Level: Intermediate
What I learned:
    Modern graphic generate animation by erasing entirewindows and redrawing it 30 or 60 times a sec with
    a frame rate of 30 or 60 frames per second (FPS)
    The higher the FPS the more fluid/smooth the animation appears

    Drawing in terminal window is slower 3 or 4 FPS causes flickering
    Need draw space characters to places where fish, kelp, bubbles located
"""
...
# (!) Try changing the constants to create a fish tank with only kelp, or only bubblers
