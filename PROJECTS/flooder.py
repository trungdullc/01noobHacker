...
class Flooder():
    ...
    def displayBoard(self, board, displayMode):
    ...
    def askForPlayerMove(self, displayMode):
    ...
    def changeTile(self, tileType, board, x, y, charToChange=None):
        ...
        if x > 0 and board[(x - 1, y)] == charToChange:
            ...
            self.changeTile(tileType, board, x - 1, y, charToChange)
        if y > 0 and board[(x, y - 1)] == charToChange:
            # Recursive Case: Change the top neighbor's tile:
            self.changeTile(tileType, board, x, y - 1, charToChange)
